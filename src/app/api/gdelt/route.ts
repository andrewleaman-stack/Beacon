...
/**
 * BEACON — Real-Time Geopolitical Events (GDELT 2.0 GeoJSON API)
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      source: allEvents[0]?.id?.includes('fb') ? 'BEACON Simulated Incident Engine' : 'GDELT 2.0 GeoJSON API',
    }, {
      headers: { 'Cache-Control': 'public, s-maxage=300, stale-while-revalidate=600' },
    });
  } catch (error) {
    console.error('[BEACON] GDELT fetch error:', error);
