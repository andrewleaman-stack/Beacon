...
/**
 * BEACON — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'BEACON/4.2', 'Accept': 'application/json' },
    });

    console.log('[BEACON] IODA response status:', res.status);
    ...
    console.error('[BEACON] IODA fetch error:', error);
