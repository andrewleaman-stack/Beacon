 ...
 * ║                                                                 ║
 * ║  Subscribes to BEACON feeds + external providers (Lattice),     ║
...
// ── BEACON Feed → Entity Translators ───────────────────────────────
...
function translateFlights(flights: any[], subtype: string): PolybolosEntity[] {
  ...
  return flights.map((f: any) => ({
    id: `beacon-air-${f.icao24 || f.callsign || Math.random().toString(36).slice(2)}`,
    ...
    source: { provider: 'beacon', feed: `flights-${subtype}`, originalId: f.icao24, confidence: 0.9 },
...
function translateMaritime(ships: any[]): PolybolosEntity[] {
  ...
  return ships.map((s: any) => ({
    id: `beacon-sea-${s.mmsi || s.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'maritime-ais', originalId: s.mmsi?.toString(), confidence: 0.85 },
    timestamp: new Date().toISOString(),
...
function translateEarthquakes(events: any[]): PolybolosEntity[] {
  ...
  return events.map((eq: any) => ({
    id: `beacon-event-eq-${eq.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'usgs-earthquakes', originalId: eq.id, confidence: 0.99 },
    timestamp: new Date().toISOString(),
...
function translateSatellites(sats: any[]): PolybolosEntity[] {
  ...
  return sats.map((s: any) => ({
    id: `beacon-space-${s.noradId || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'satnogs', originalId: s.noradId?.toString(), confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
function translateFires(fires: any[]): PolybolosEntity[] {
  ...
  return fires.map((f: any, i: number) => ({
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'nasa-firms', confidence: 0.9 },
    timestamp: new Date().toISOString(),
...
function translateCCTV(cameras: any[]): PolybolosEntity[] {
  ...
  return cameras.map((c: any) => ({
    id: `beacon-sensor-cctv-${c.id || Math.random().toString(36).slice(2)}`,
    name: c.name || 'Camera',
    domain: Domain.LAND,
    ...
    threat: ThreatLevel.NONE,
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'cctv-network', originalId: c.id, confidence: 1.0 },
    timestamp: new Date().toISOString(),
...
function translateRadiation(stations: any[]): PolybolosEntity[] {
  ...
  return stations.map((r: any) => ({
    id: `beacon-sensor-rad-${r.name || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'beacon', feed: 'radiation-network', confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
export class PolybolosClient {
  ...
  /** Connect to the BEACON SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      this.sseConnection = new EventSource(
        `${this.config.beaconBaseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw BEACON data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestBeaconData(data: Record<string, any>): void {
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count BEACON feeds that have data
